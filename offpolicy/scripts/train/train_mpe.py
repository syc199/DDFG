...
from offpolicy.utils.util import get_cent_act_dim, get_dim_from_space,get_list_from_space
...
def parse_args(args, parser):
    ...
    parser.add_argument('--num_collectors', type=int,
                        default=3, help="number of collectors for Cooperative Treasure Collection")
...
def main(args):
    ...
    if all_args.share_policy:
        policy_info = {
            'policy_0': {"cent_obs_dim": get_dim_from_space(env.share_observation_space[0]),
                         ...
                         "obs_dim_list": get_list_from_space(env.observation_space)}
        }
    ...
    # choose algo
    #print(all_args.algorithm_name)


    if all_args.algorithm_name in ["rmatd3", "rmaddpg", "rmasac", "qmix", "vdn","rmdfg","rmdfg_cent","rddfg_cent_rw"]:
        ...
        eval_env = env
    elif all_args.algorithm_name in ["matd3", "maddpg", "masac", "mqmix", "mvdn","mdfg"]:
        from offpolicy.runner.mlp.mpe_runner import MPERunner as Runner
        eval_env = make_eval_env(all_args)
    else:
        raise NotImplementedError
    adj = np.zeros((all_args.num_agents,all_args.num_factor))
    index = 0
    if all_args.use_dyn_graph == False and all_args.equal_vdn == False:
        '''for i in range(all_args.num_agents // 2):
            for j in range(all_args.num_agents // 2,all_args.num_agents):
                adj[i,index] = 1
                adj[j,index] = 1
                index = index + 1'''
    ...
    df = pd.DataFrame(columns=['step','reward','min_dists','num_collision'])
    df.to_csv(progress_filename,index=False)
    
    progress_filename_train = os.path.join(run_dir,'progress_train.csv')
    #df = pd.DataFrame(columns=['step','loss','grad_norm','Q_tot']) 
    df = pd.DataFrame(columns=['step','critic_loss','critic_grad_norm','actor_loss','actor_grad_norm']) 
    df.to_csv(progress_filename_train,index=False)
    
    progress_filename_train = os.path.join(run_dir,'progress_train_adj.csv')
    df = pd.DataFrame(columns=['step','rl_loss','adj_loss','adj_grad_norm']) 
    df.to_csv(progress_filename_train,index=False)
