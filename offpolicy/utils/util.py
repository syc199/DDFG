...
def log_loss(e,eps):
    e_clamp = torch.clamp((e**2).mean(dim=0),eps,1-eps)
    
    return 1. / -torch.log(e_clamp)

def adj_init(module, weight_init, bias_init):
...
def sample_gumbel(shape, eps=1e-20, tens_type=torch.FloatTensor,re_grad=False):
    """Sample from Gumbel(0, 1)"""
    U = Variable(tens_type(*shape).uniform_(), requires_grad=re_grad)
    return -torch.log(-torch.log(U + eps) + eps)

# modified for PyTorch from https://github.com/ericjang/gumbel-softmax/blob/master/Categorical%20VAE.ipynb
def gumbel_softmax_sample(logits, avail_logits, temperature, device=torch.device('cpu'),r_grad=False):
    """ Draw a sample from the Gumbel-Softmax distribution"""
    if str(device) == 'cpu':
        y = logits + sample_gumbel(logits.shape, tens_type=type(logits.data),re_grad=r_grad)
    else:
        y = (logits.cpu() + sample_gumbel(logits.shape,
                                          tens_type=type(logits.data),re_grad=r_grad)).cuda()
...
def gumbel_softmax_mdfg(logits, temperature, hard=False, num_factor=6 ,highest_orders=1,device=torch.device('cpu'),requires_grad=False):
    """Sample from the Gumbel-Softmax distribution and optionally discretize.
      Args:
        logits: [batch_size, n_class] unnormalized log-probs
        temperature: non-negative scalar
      ...
      Returns:
        ...
        be a probabilitiy distribution that sums to 1 across classes
        """
    y = gumbel_softmax_sample(logits, None, temperature, device, requires_grad)
    y_T = y.reshape(-1,num_factor,2).transpose(0,1)
    num_agent = y_T.shape[1]
    y_index = torch.argsort(y_T[:,:,1],1)
    yr = torch.max(y_T,2)[0]
    
    if hard:
        y_hard = torch.max(y_T.data, 2)[1]
        #import pdb;pdb.set_trace()
        limit = torch.min(y_hard.sum(dim=1),torch.ones(num_factor,dtype=torch.int64).to(device)*3)
        y_hard.zero_()
        for i in range(num_factor):
            if limit[i] == 0:
                limit[i] = 1
            y_hard[i,y_index[i,:limit[i]]] = 1

        yr = (y_hard - yr).detach() + yr
    return yr.transpose(0,1)
    #torch.cat([y.transpose(0,1),torch.eye(num_agent,num_agent).long().to(device)],dim=1)
