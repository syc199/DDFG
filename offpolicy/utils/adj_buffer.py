...
class AdjBuffer(object):
    ...
    def sample(self, batch_size,data_chunk_length):
        ...
        for p_id in self.policy_info.keys():
            obs_batch[p_id], share_obs_batch[p_id], dones_batch[p_id], dones_env_batch[p_id], adj_batch[p_id], prob_adj_batch[p_id], advantages_batch[p_id], f_advts_batch[p_id], rnn_obs_batch[p_id] = self.policy_buffers[p_id].sample_inds(inds,data_chunk_length)
...
class AdjPolicyBuffer(object):
    def __init__(self, buffer_size, episode_length, num_agents, num_factor, obs_space, share_obs_space, act_space,
        ...
        self.f_q = np.zeros((self.episode_length, self.buffer_size, self.num_factor, 1), dtype=np.float32)
        self.advantage = np.zeros((self.episode_length, self.buffer_size, 1), dtype=np.float32)
        self.f_v = np.zeros((self.episode_length, self.buffer_size, self.num_factor, 1), dtype=np.float32)
    ...
    def compute_advantage(self, idx, value_normalizer=None):
        ...
        for step in reversed(range(0,self.rewards.shape[0])):
            ...
            f_delta = self.f_q[step,idx] -  self.f_v[step,idx]
    ...
    def sample_inds(self, sample_inds, data_chunk_length):
        """
        ...
        f_advt = (f_advt - mean_advt_f) / (std_advt_f + 1e-10)
