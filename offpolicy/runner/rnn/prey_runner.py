...
class PREYRunner(RecRunner):
    ...
    def collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        episode_f_v = {p_id : np.zeros((self.episode_length, self.num_envs, self.num_factor, 1), dtype=np.float32) for p_id in self.policy_ids}
        episode_f_q = {p_id : np.zeros((self.episode_length, self.num_envs, self.num_factor, 1), dtype=np.float32) for p_id in self.policy_ids}
            ...
            if self.algorithm_name in self.adj_correlation:
                ...
                # get new rnn hidden state
                    ...
                    if self.use_dyn_graph:   
                        prob_adj, adj, _, _ =  self.adj_network.sample(rnn_states_batch.unsqueeze(0),states_batch[None,:],explore,self.total_env_steps)
                        prob_adj = prob_adj[0]
                        adj_all = torch.cat([adj,torch.eye(self.num_agents,dtype=torch.int64).unsqueeze(0)],dim=2)
                        adj = adj[0]
                    else:
                    ...
                    _, rnn_states_batch ,_ = policy.get_hidden_states(obs_batch,last_acts_batch,rnn_states_batch) 
                    if self.use_dyn_graph:   
                        prob_adj, adj, _, _ =  self.adj_network.sample(rnn_states_batch.unsqueeze(0),states_batch,explore,self.total_env_steps)
                        #prob_adj, adj, _, _ =  self.adj_network.sample(obs_batch[None,:],states_batch[None,:])
                        prob_adj = prob_adj[0]
                        adj_all = torch.cat([adj,torch.eye(self.num_agents,dtype=torch.int64).unsqueeze(0)],dim=2)
                        adj = adj[0]
                    else:
                    ...
                    if self.use_vfunction:
                        f_v = policy.get_v_values(rnn_states_batch.unsqueeze(0) ,adj_all)
        ...
        if self.algorithm_name in self.adj_correlation:
            ...
            if self.use_dyn_graph:   
                prob_adj, adj,_, _ =  self.adj_network.sample(rnn_states_batch.unsqueeze(0),np.concatenate(share_obs),explore,self.total_env_steps)
                prob_adj = prob_adj[0]
                adj_all = torch.cat([adj,torch.eye(self.num_agents,dtype=torch.int64).unsqueeze(0)],dim=2)
                adj = adj[0]
            else:
        ...
        #actions = episode_acts[p_id].argmax(axis=-1)[1]
