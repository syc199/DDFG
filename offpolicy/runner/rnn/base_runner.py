...
class RecRunner(object):
    ...
    def __init__(self, config):
        ...
        self.adj = config["adj"]
        self.q_learning = ["qtran","qmix","vdn","rddfg_cent_rw","dcg"]
        self.adj_correlation = ["rddfg_cent_rw","dcg"]
        ...
        if self.algorithm_name == "rmaddpg":
        ...
        elif self.algorithm_name == "dcg":
            from offpolicy.algorithms.dcg.algorithm.adj_generator import Adj_Generator as Adj
            from offpolicy.algorithms.dcg.algorithm.DCGPolicy import DCGPolicy as Policy
            from offpolicy.algorithms.dcg.dcg import DCG as TrainAlgo
    ...
    def run(self):
        ...
        if self.use_dyn_graph and ((self.num_episodes_collected - self.last_train_adj_episode - self.batch_size) / self.train_adj_episode) >= 1 :    
    ...
    def batch_train_adj(self):
        ...
        for _ in range(10):
            sample = self.adj_buffer.sample(self.batch_size,data_chunk_length)
            train_adj_info, new_priorities, idxes = self.trainer.train_adj_on_batch(sample,self.total_env_steps)
