...
class MPERunner(RecRunner):
    ...
    def __init__(self, config):
        ...
        if self.share_policy:
            self.collecter = self.shared_collect_rollout_mdfg if self.algorithm_name == "rmdfg" else self.shared_collect_rollout
        else:
    ...
    def shared_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        episode_num_collision = {p_id : np.ones((self.episode_length, self.num_envs, self.num_agents, 1), dtype=np.float32) for p_id in self.policy_ids}
        episode_min_dist = {p_id : np.ones((self.episode_length, self.num_envs, 1), dtype=np.float32) for p_id in self.policy_ids}
        ...
        while t < self.episode_length:
            ...
            if self.algorithm_name == "rmdfg_cent"  or self.algorithm_name == "rddfg_cent_rw":
                    if self.use_dyn_graph:   
                        prob_adj, adj, _ =  self.adj_network.sample(obs_batch)
                    else:
            ...
            if warmup:
                ...
                _, rnn_states_batch, _ = policy.get_actions(obs_batch,  #(3,18)
                                                            last_acts_batch, #(3,5)
                                                            rnn_states_batch, #(3,64)
                ...
                if self.algorithm_name == "rmdfg_cent"  or self.algorithm_name == "rddfg_cent_rw":
                    acts_batch, rnn_states_batch, _ = policy.get_actions(obs_batch,
                                                                    ...
                                                                    explore=explore,
                                                                    adj_input =  torch.eye(self.num_agents,dtype=torch.int64) if self.equal_vdn else torch.cat([adj,torch.eye(self.num_agents,dtype=torch.int64)],dim=1)) 
                else:
                    acts_batch, rnn_states_batch, _ = policy.get_actions(obs_batch,
                                                                    last_acts_batch,
                                                                    rnn_states_batch,
            ...
            episode_num_collision[p_id][t] = infos[:,:,:,0]
            episode_min_dist[p_id][t] = infos[:,0,:,1]
            t += 1
            obs = next_obs
        ...
        num_collision = np.sum(episode_num_collision[p_id]) / 2

        env_info['average_episode_rewards'] = average_episode_rewards
        env_info['min_dists'] = episode_min_dist[p_id][self.episode_length-1,0,0]
        env_info['num_collision'] = num_collision
        return env_info

    @torch.no_grad()
    def shared_collect_rollout_mdfg(self, explore=True, training_episode=True, warmup=False):
        env_info = {}
        # only 1 policy since all agents share weights
        p_id = "policy_0"
        policy = self.policies[p_id]
        obs_dim_list = policy.obs_dim_list
        env = self.env if training_episode or warmup else self.eval_env
        ...
        episode_dones_env = {p_id : np.ones((self.episode_length, self.num_envs, 1), dtype=np.float32) for p_id in self.policy_ids}
        episode_avail_acts = {p_id : None for p_id in self.policy_ids}
        episode_adj = {p_id : np.zeros((self.num_envs, self.num_agents, self.num_factor), dtype=np.int64) for p_id in self.policy_ids}

        if self.algorithm_name == "rmdfg":
            if self.use_dyn_graph:
                adj =  self.adj_network.sample(True).long().cpu()
            else:
                adj = torch.tensor(self.adj,dtype=torch.int64)
        t = 0
        while t < self.episode_length:
            ...
            for i in range(self.num_agents):
                episode_obs[p_id][t][0,i,:obs_dim_list[i]] = obs[0,i]
            if share_obs.shape[1] == self.num_agents:
                for i in range(self.num_agents):
                    episode_share_obs[p_id][t][0,i] = np.concatenate(share_obs[0])
            else:
                for i in range(self.num_agents):
                    episode_share_obs[p_id][t][0,i] = np.concatenate(share_obs)
            episode_acts[p_id][t] = np.stack(env_acts)
            episode_rewards[p_id][t] = np.repeat(rewards.mean(),6,axis=0)[None,:,None]
            episode_dones[p_id][t] = dones
            episode_dones_env[p_id][t] = dones_env
            episode_num_collision[p_id][t] = infos[:,:,:,0]
            episode_min_dist[p_id][t] = infos[:,0,:,1]
    ...
    def separated_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        episode_num_collision = {p_id : np.ones((self.episode_length, self.num_envs, len(self.policy_agents[p_id]), 1), dtype=np.float32) for p_id in self.policy_ids}
        episode_min_dist = {p_id : np.ones((self.episode_length, self.num_envs, 1), dtype=np.float32) for p_id in self.policy_ids}
        episode_dones_env = {p_id : np.ones((self.episode_length, self.num_envs, 1), dtype=np.float32) for p_id in self.policy_ids}
        episode_avail_acts = {p_id : None for p_id in self.policy_ids}
        episode_adj = {p_id : np.zeros((self.num_envs, len(self.policy_agents[p_id]), self.num_factor), dtype=np.int64) for p_id in self.policy_ids}
        ...
        while t < self.episode_length:
            for agent_id, p_id in zip(self.agent_ids, self.policy_ids):
                ...
                act = act if isinstance(act, np.ndarray) else act.cpu().detach().numpy()
                last_acts[p_id] = np.expand_dims(act, axis=1)
            ...
            for agent_id, p_id in zip(self.agent_ids, self.policy_ids):
                episode_rewards[p_id][t] = np.expand_dims(np.mean(rewards,axis=1),axis=0)#np.expand_dims(rewards[:, agent_id], axis=1)
                episode_dones[p_id][t] = np.expand_dims(dones[:, agent_id], axis=1)
                episode_dones_env[p_id][t] = dones_env
                episode_num_collision[p_id][t] = np.expand_dims(infos[:, agent_id,:,0], axis=1)
                episode_min_dist[p_id][t] = np.expand_dims(infos[:, 0,:,1], axis=1)
        ...
        if explore:
            self.num_episodes_collected += self.num_envs
            self.buffer.insert(self.num_envs, episode_obs, episode_share_obs, episode_acts, episode_rewards, episode_dones, episode_dones_env, episode_avail_acts,episode_adj)
        ...
        for p_id in self.policy_ids:
            ...
            num_collision.append(np.mean(np.sum(episode_num_collision[p_id], axis=0)) / 2)

        env_info['average_episode_rewards'] = np.mean(average_episode_rewards)
        #print(np.mean(average_episode_rewards))
        env_info['min_dists'] = episode_min_dist['policy_0'][self.episode_length-1,0,0]
        env_info['num_collision'] = np.mean(num_collision) / self.num_agents
        return env_info
    ...
    def log_clear(self):
        ...
        self.env_infos['min_dists'] = []
        self.env_infos['num_collision'] = []
