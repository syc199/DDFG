...
class R_DDFGPolicy(MLPPolicy):
    ...
    def get_v_batch(self,obs_batch,adj_input=None,batch_size=1,no_sequence=False):
    ...
    def get_v_values(self, obs_batch,adj_input=None,no_sequence=False):
        ...
        v_batch, idx_node_order, adj , num_edges= self.get_v_batch(obs_batch, adj_input,batch_size,no_sequence) #0.00358
        ...
        if batch_size == 1:
            ...
            for i in range(len(idx_node_order)):
                if i == 0:
                    v_batch[i] = v_batch[i] / self.n_agents
                if len(idx_node_order[i]) != 0:
    ...
    def v_local_values(self, f_q, idx_node_order):
        ...
        return values_f[:,:self.num_factor-self.n_agents]
      
    def get_rnn_batch(self,obs_batch,adj_input=None,batch_size=1,no_sequence=False):
    ...
    def get_q_values(self, obs_batch, action_batch, adj_input=None,no_sequence=False,available_actions=None):
        """
        ...
        q_batch, idx_node_order, adj , num_edges= self.get_rnn_batch(obs_batch, adj_input,batch_size,no_sequence) #0.00358
    ...
    def q_values(self, f_q, actions, idx_node_order,batch_size):
        ...
        if len(idx_node_order[0]) != 0:
            ...
            #indexn = torch.cat([idx_a_of_Q[0][1],torch.tensor([batch_size]).to(self.device)])
            
            num1 = torch_scatter.scatter_add(src=torch.ones_like(tmp1), index=index1, dim=0)[:-1].reshape(-1,batch_size)
            n1 = torch.where(num1==0,torch.ones_like(num1),num1)
            values[0] = torch_scatter.scatter_add(src=tmp1, index=index1, dim=0)[:-1].reshape(-1,batch_size) / n1
        ...
        if len(idx_node_order[1]) != 0:
            ...
            num2 = torch_scatter.scatter_add(src=torch.ones_like(tmp2), index=index2, dim=0)[:-1].reshape(-1,batch_size)
            n2 = torch.where(num2==0,torch.ones_like(num2),num2)
            values[1] = torch_scatter.scatter_add(src=tmp2, index=index2, dim=0)[:-1].reshape(-1,batch_size)  / n2
        ...
        if self.highest_orders == 3 and len(idx_node_order[2]) != 0:
            ...
            num3 = torch_scatter.scatter_add(src=torch.ones_like(tmp3), index=index3, dim=0)[:-1].reshape(-1,batch_size)
            n3 = torch.where(num3==0,torch.ones_like(num3),num3)
            values[2] = torch_scatter.scatter_add(src=tmp3, index=index3, dim=0)[:-1].reshape(-1,batch_size) / n3
        # Return the Q-values for the given actions
        return values.sum(0)
    ...
    def greedy(self, adj, q_batch, idx_node_order,available_actions,num_edges,batch_size):
        ...
        var_edge = torch.where(adj.reshape(-1,self.num_factor))[0]
        f_edge = torch.where(adj.reshape(self.n_agents,-1))[1]
        ...
        for i in range(len(idx_node_order)):
            ...
            if len(idx_node_order[i]) != 0:
                ...
                q_batch[i] = q_batch[i].reshape((-1,q_batch[i].shape[0],self.act_dim,self.act_dim,self.act_dim))
        ...
        best_f_value = torch.empty(batch_size, self.num_factor-self.n_agents,1, dtype=torch.float64).fill_(-float('inf')).to(**self.tpdv)
    ...
    def get_actions(self, obs_batch, available_actions=None, t_env=None, explore=False,adj_input = None,no_sequence = False):
        ...
        q_batch, idx_node_order, adj, num_edges = self.get_rnn_batch(obs_batch,adj_input,batch_size,no_sequence)
