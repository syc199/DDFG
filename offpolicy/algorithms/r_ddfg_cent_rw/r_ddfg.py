...
class R_DDFG:
    def __init__(self, args, num_agents, policies, adj_network, policy_mapping_fn, device=torch.device("cuda:0"), episode_length=25,vdn=False):
        ...
        self.adj_lambda = self.args.adj_lambda
    ...
    def train_policy_on_batch(self, batch, use_same_share_obs=None):
        ...
        for p_id in self.policy_ids:
            ...
            policy_qs = policy.get_q_values(obs_q,curr_act_batch_ind,adj_input_q,no_sequence)
            ...
            with torch.no_grad():
                ...
                greedy,_,_,_ = policy.get_actions(obs_qtot, curr_avail_act, None, False, adj_input_qtot, no_sequence) 
                curr_nact_batch_ind = torch.from_numpy(greedy).max(dim=-1)[1].to(self.device)
                target_policy_qs = target_policy.get_q_values(target_obs, curr_nact_batch_ind.unsqueeze(dim=-1), adj_input_qtot,no_sequence)
            ...
            if self.use_vfunction:
                policy_v = policy.get_v_values(obs_q,adj_input_q,no_sequence)
                with torch.no_grad():
                    target_policy_v =target_policy.get_v_values(target_obs,adj_input_qtot,no_sequence)
    ...
    def train_adj_on_batch(self, batch, total_env_steps, use_same_share_obs=None):
        """See parent class."""
        ...
        for p_id in self.policy_ids:
            ...
            policy = self.policies[p_id]
            # get data related to the policy id
            adj = to_torch(adj_batch[p_id])#[batch_size,step,num_agent,-1]
            step = adj.shape[1]     
            batch_size = adj.shape[0] * adj.shape[1] 
            adj = adj.reshape(batch_size,self.num_agents,-1).to(**self.tpdv) 
            state_batch = to_torch(share_obs_batch[p_id]).reshape(batch_size,-1).to(**self.tpdv)  
            dones = to_torch(dones_batch[p_id]).reshape(batch_size,self.num_agents,-1).to(**self.tpdv)
            dones_env = to_torch(dones_env_batch[p_id]).reshape(batch_size,-1).to(**self.tpdv)
            prob_adj = to_torch(prob_adj_batch[p_id]).reshape(batch_size,self.num_agents,-1).to(**self.tpdv)
            f_advts = to_torch(f_advts_batch[p_id]).reshape(batch_size,self.num_factor,-1).to(**self.tpdv)
            rnn_obs= to_torch(rnn_obs_batch[p_id]).reshape(batch_size,self.num_agents,-1).to(**self.tpdv) #[batch_size,step-1,1]
            _, target_adj , entropy, target_prob_adj  =  self.adj_network.sample(rnn_obs,state_batch)
            tar_prob_adj = torch.log(torch.where(adj==1,target_prob_adj,torch.ones_like(target_prob_adj,dtype=torch.float32)))
            #import pdb;pdb.set_trace()
            adj_entropy.append(entropy)
            tarprob_adj.append(tar_prob_adj)
            
        #advantage = target_Q_tot.unsqueeze(-1)
        target_prob_adj = torch.cat(tarprob_adj, dim=-1)

        adj_entropy_batch = torch.cat(adj_entropy, dim=-1).unsqueeze(-1)
        diff_log = torch.clamp(target_prob_adj.sum(-2)-prob_adj.sum(-2),max=80)   
        imp_weights = torch.exp(diff_log).unsqueeze(-1)
        bad_transitions_mask = dones_env
        clamp_imp_weights = torch.clamp(imp_weights, 1.0 - self.clip_param, 1.0 + self.clip_param)
        surr1 = imp_weights * f_advts
        ...
        loss.backward()
        grad_norm = torch.nn.utils.clip_grad_norm_(self.adj_parameters, 0.5)
